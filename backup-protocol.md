...
The following terminal command is the **only** approved method for creating a backup:

```
zip -r backup_$(date +%Y-%m-%d).zip . -x "*.zip"
```

### Command Breakdown:

- `zip -r`: Creates a recursive ZIP archive.
- `backup_$(date +%Y-%m-%d).zip`: Names the archive with the prefix "backup_" followed by the current date in `YYYY-MM-DD` format.
